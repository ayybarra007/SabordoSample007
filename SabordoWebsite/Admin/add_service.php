...
$check_title = mysqli_query($connection, "SELECT * FROM tbl_services WHERE Title='$title'");
$check_title_row = mysqli_num_rows($check_title);

if($check_title_row > 0) {

$titleErr = "Title is already exists!...";

}else {

$query = mysqli_query($connection, "INSERT INTO tbl_services(Title,Description) VALUES('$title','$desc')");

echo "<script language='javascript'>alert('New Services has been Added!')</script>";
...
body {
  ...
  <div class="d-flex flex-column flex-shrink-0 p-3 text-white bg-dark" id="sidebar">
    ...
    <ul class="nav nav-pills flex-column mb-auto">
      ...
      <li class="nav-item">
        <a href="add_service.php" class="nav-link active" data-target="services">
...
<form style="text-align: center; padding: 15px 20px; font-family: sans-serif;" method="POST" action="<?php htmlspecialchars("PHP_SELF"); ?>">
   ...
   Title<br><input style="margin: 20px; border-radius: 5px; border: none; width:30%; height:30px;" type="text" name="Title" value="<?php echo $title; ?>"> <br>
   ...
   <input style="text-decoration: none;
    ...
    cursor:pointer;" type="submit" name="submit" value="Add Services">
